...
def get_policy_from_aws(policy_identifier: str) -> Dict[str, Any]:
    ...
    try:
        ...
        policy_document = iam.get_policy_version(PolicyArn=policy["Policy"]["Arn"], VersionId=policy_version)[
            "PolicyVersion"
        ]["Document"]

        logging.info(f"Fetched policy: {policy['Policy']['PolicyName']} ({policy['Policy']['Arn']})")
        return policy_document  # type: ignore[no-any-return]
    except iam.exceptions.NoSuchEntityException:
...
def get_policies_from_role(role_name: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for policy in attached_policies["AttachedPolicies"]:
            ...
            policies.append(
                {"PolicyName": policy_name, "PolicyArn": policy_arn, "PolicyType": "Managed", "Document": policy_doc}
            )
